...
namespace SystemSage {
  ...
  sealed class FileSnapshot { public string Path; public long Length; public DateTime Modified; }
  ...
  static class FullDiagnostic {
    public static DiagnosticReport Collect(Action<int,string> progress=null) {
      ...
      Step(progress,74,"Reading printers");
      Add(report,"Printers",new[]{"Name","Default","Status","Port","Driver"},Scan.Printers());
      Step(progress,76,"Reading Bluetooth devices");
      Add(report,"Bluetooth",new[]{"Name","Status","Class","Device ID"},Scan.Bluetooth());
      Step(progress,78,"Reading USB devices");
      Add(report,"USB devices",new[]{"Name","Manufacturer","Status","Class","Device ID"},Scan.UsbDevices());
      Step(progress,80,"Checking firewall profiles");
      Add(report,"Firewall",new[]{"Profile","State","Inbound","Outbound"},Scan.Firewall());
      Step(progress,82,"Checking BitLocker status");
      Add(report,"BitLocker",new[]{"Volume","Protection","Conversion","Method"},Scan.BitLocker());
      Step(progress,84,"Reading restore points");
      Add(report,"Restore points",new[]{"Item","Description","Created","Type"},Scan.RestorePoints());
      Step(progress,86,"Estimating installed app sizes");
      Add(report,"Installed apps (top)",new[]{"Name","Size","Version","Publisher"},Scan.InstalledApps(20));
      Step(progress,88,"Reading thermal zones");
      Add(report,"Thermal zones",new[]{"Zone","Celsius","Kelvin","Source"},Scan.Thermal());
      Step(progress,90,"Reading proxy and hosts");
      Add(report,"Proxy and hosts",new[]{"Property","Value"},Scan.ProxyAndHosts());
      Step(progress,91,"Reading Outlook accounts and storage");
      ...
      Step(progress,99,"Finalizing diagnostic results");
      return report;
    }
    ...
    static List<string[]> RecentErrors(){var rows=new List<string[]>();DateTime since=DateTime.Now.AddDays(-7);foreach(string logName in new[]{"System","Application"}){try{using(var log=new EventLog(logName)){int found=0;for(int i=log.Entries.Count-1;i>=0&&found<15;i--){EventLogEntry e=log.Entries[i];if(e.TimeGenerated<since)break;if(e.EntryType!=EventLogEntryType.Error&&e.EntryType!=EventLogEntryType.Warning)continue;string message=Clean(e.Message);if(message.Length>700)message=message.Substring(0,697)+"...";rows.Add(new[]{e.TimeGenerated.ToString("yyyy-MM-dd HH:mm"),logName,e.Source,e.InstanceId.ToString(),message});found++;}}}catch(Exception e){rows.Add(new[]{"Unavailable",logName,"SystemSage","-",e.Message});}}return rows.OrderByDescending(x=>x[0]).Take(25).ToList();}
    ...
    static List<string[]> LargestFiles(string root,int limit,Action<int,string> progress){var files=new List<FileSnapshot>();var pending=new Stack<string>();pending.Push(root);var timer=Stopwatch.StartNew();int visited=0,nextUpdate=2500;while(pending.Count>0&&visited<250000&&timer.Elapsed<TimeSpan.FromSeconds(60)){string dir=pending.Pop();try{foreach(string file in Directory.EnumerateFiles(dir)){if(visited>=250000||timer.Elapsed>=TimeSpan.FromSeconds(60))break;visited++;if(visited>=nextUpdate){Step(progress,Math.Min(94,88+visited/42000),"Scanning user files - "+visited.ToString("N0")+" checked");nextUpdate+=2500;}try{var info=new FileInfo(file);files.Add(new FileSnapshot{Path=info.FullName,Length=info.Length,Modified=info.LastWriteTime});if(files.Count>limit*20)files=files.OrderByDescending(x=>x.Length).Take(limit*4).ToList();}catch{}}foreach(string child in Directory.EnumerateDirectories(dir)){try{var a=File.GetAttributes(child);if((a&FileAttributes.ReparsePoint)==0)pending.Push(child);}catch{}}}catch{}}Step(progress,94,"Largest-file scan complete - "+visited.ToString("N0")+" checked");return files.OrderByDescending(x=>x.Length).Take(limit).Select(x=>new[]{x.Path,Scan.Bytes(x.Length),x.Modified.ToString("yyyy-MM-dd HH:mm")}).ToList();}
