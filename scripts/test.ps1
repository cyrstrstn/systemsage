...
$commands=@(@('--version'),@('doctor'),@('memory'),@('gpu'),@('diskhealth'),@('processes'),@('storage'),@('wifi'),@('boot'),@('display'),@('audio'),@('browsers'),@('printers'),@('bluetooth'),@('usb'),@('firewall'),@('bitlocker'),@('restore'),@('apps'),@('thermal'),@('proxy'),@('temp'),@('outlook'),@('security'),@('battery'),@('network'),@('startup'),@('system'),@('json'))
...
& $exe @('baseline','--save') 2>&1|Out-Null; if($LASTEXITCODE -ne 0){throw 'baseline --save failed'}; Write-Host "PASS systemsage baseline --save" -ForegroundColor Green
$output=& $exe @('compare') 2>&1;if($LASTEXITCODE -ne 0){throw "Command failed: compare`n$output"};Write-Host "PASS systemsage compare" -ForegroundColor Green
